...
- `request_attachments` for MinIO-backed file attachments with presigned upload and preview flows

`request_assignees` is the source of truth for current assignment membership. `assigned_to` remains a primary-assignee compatibility field for legacy response shape compatibility. Pool, done, and dashboard request reads are resolved through database RPCs using `request_assignees` membership checks, not by loading all assignment rows into the API process.

## Request Attachments

Request attachments allow files to be attached during request creation or done reply submission. The service module `app/services/request_attachment_service.py` orchestrates the upload flow with MinIO presigned URLs. Attachments are grouped by context (`request` or `done_reply`) and included in request detail responses. The route is mounted at `/request-attachments`.
...
- Request update is implemented through `PATCH /requests/{request_id}`; empty update payloads return the unchanged request, while non-empty payloads update through the repository.
- Request attachments support presigned upload and preview flows for both request creation and done reply contexts.
- Backend request workflow creates assignment history, status logs, and notifications.
- Request assignment membership writes are no longer swallowed; if `request_assignees` cannot be written, create/self-assign workflows fail instead of silently diverging from the assignment source of truth.
...
The app is suitable for internal beta use after the current request workflow regression fixes, provided it is operated with restricted access, correct production env values, and regular backups. Remaining production hardening work is tracked in `docs/internal-beta-readiness.md`.
