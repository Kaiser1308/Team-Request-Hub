...
Create request payload supports optional multi-assignee assignment, reference links, and attachment IDs:
...
```json
{
  ...
  "reference_links": ["https://example.com/issue/1"],
  "attachment_ids": ["attachment-uuid-1"]
}
```

Update request supports optional `reference_links`:

```json
{
  "title": "Updated title",
  "reference_links": ["https://example.com/issue/2"]
}
```

Request response includes plural assignees and grouped attachments:
...
`POST /requests/{request_id}/done` supports optional `attachment_ids`:
...
`POST /requests/{request_id}/attachments` — add already-uploaded `request`-context attachments to an open request. Creator or lead only. Body: `{ "attachment_ids": ["uuid", ...] }`. Enforces the 5-attachment cap. Returns the request's grouped attachments (`{ "request": [...], "done_reply": [...] }`).

`DELETE /requests/{request_id}/attachments/{attachment_id}` — remove a `request`-context attachment from an open request. Creator or lead only. Hard-deletes the MinIO object and records an audit entry. Returns the updated grouped attachments.
...
```txt
POST /request-attachments/upload-url
POST /request-attachments/{attachment_id}/complete-upload
POST /request-attachments/{attachment_id}/preview-url
POST /request-attachments/{attachment_id}/download-url
```

`POST /request-attachments/upload-url` — request a presigned upload URL for a request attachment.
...
`context` is either `request` (attached during creation) or `done_reply` (attached during done reply).
...
`POST /request-attachments/{attachment_id}/complete-upload` — confirm upload completed. Request: `{ "size_bytes": 2048 }`.

`POST /request-attachments/{attachment_id}/preview-url` — get a presigned preview URL.
...
`POST /request-attachments/{attachment_id}/download-url` — get a presigned download URL with `Content-Disposition: attachment` header for forced file download. Same response shape as preview-url.
