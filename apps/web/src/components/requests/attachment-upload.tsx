    ...
    >
      ...
      {hook.files.length > 0 && (
        <ul className="space-y-1">
          {hook.files.map((pf) => (
            <FileItem key={pf.id} pf={pf} onRemove={() => hook.removeFile(pf.id)} />
          ))}
        </ul>
      )}
    </div>
...
function FileItem({ pf, onRemove }: { pf: PendingFile; onRemove: () => void }) {
  return (
    <li className="flex items-center gap-2 rounded-md border border-[#e5e7eb] bg-gray-50 px-3 py-2 text-sm">
      ...
      {pf.status === "done" && (
        <span className="shrink-0 text-xs text-green-600">OK</span>
      )}
      {pf.status === "error" && (
