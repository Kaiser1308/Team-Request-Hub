...
- `docker-compose.yml` defines the workstation deployment stack (`web`, `api`, `minio`, `minio-init`, `cloudflared`). See `docs/deployment-workstation.md` before changing deployment behavior.
...
- API entrypoint is `app/main.py`; routers are mounted at `/health`, `/users`, `/requests`, `/notifications`, `/dashboard`, `/notifications/telegram`, `/files`, and `/request-attachments`.
...
- Request business rules are implemented in backend services, including assignment, status transitions, done replies, cancellation, notification records, assignment history, status logs, and request attachments.
...
- Request attachments use MinIO presigned URLs via `app/services/request_attachment_service.py` for uploading files during request creation and done replies.
...
This project is indexed by GitNexus as **Team-Request-Hub** (2989 symbols, 5095 relationships, 167 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
